...
En la Tabla 17 podemos observar los valores únicos de esta variable, los cuales son "0" y "1". Esta variable indica si el usuario acertó la pregunta. Para mayor entendimiento de la variable, se transforma de la siguiente manera: 0=No y 1= Yes. Lo anterior se puede ver en la Tabla 18.

```{r}
#Frecuencia

kable(datos_modelo %>% group_by(acertou) %>% summarise(n = n()) %>% arrange(-n),caption = "Valores únicos - Acertou corregidos") %>% 
  kable_styling(full_width = F,position = "center") %>% 
  kable_minimal()
```

<center>**Tabla 17.** Valores únicos de la variable "acertou"</center>
...
kable(datos_modelo %>% group_by(acertou) %>% summarise(n = n()) %>% arrange(-n),caption = "Valores únicos - Acertou") %>% 
  kable_styling(full_width = F,position = "center") %>% 
  kable_minimal()
```

<center>**Tabla 18.** Valores únicos de la variable "acertou" corregidos</center>
...
En la Tabla 19 podemos observar los valores, NA por cada variable de la base de datos.
...
Se puede observar que las variables "modality_id", "scholarity_id", "nullified" y "outdated" se tiene la misma cantidad de valores NA. En la Tabla 20 se observa los registros donde se evidencia esto.
...
<center>**Tabla 20.** Registros del dataset donde las variables "modality_id", "scholarity_id", "nullified" y "outdated" son NA</center>

<br>

En la Tabla 20 podemos observar que se cumple que en los 178 registros donde las variables "modality_id", "scholarity_id", "nullified" y "outdated" son NA, son los mismos. Se procede a eliminar estos registros.

```{r}
datos_modelo <- datos_modelo[!is.na(datos_modelo$modality_id),]
```

En la Tabla 21, se vuelve a calcular la cantidad de valores NA por cada variable.
...
<center>**Tabla 21.** Cantidad de valores NA de cada variable de la base datos</center>
...
Para la variable "right_answer", los valores NA se van a

```{r}
#pepe <- datos_modelo[is.na(datos_modelo$modality_id),]
```
